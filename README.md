...
When the cache auto-generates a query key, [it's hashed](#operations-are-not-commutative), making it extremely difficult to remove it from the cache. If you need to remove the results from the cache at any given time, use a custom key.

```php
User::query()->cache(key: 'find_joe')->whereName('Joe')->whereAge(20)->first();

cache()->forget('cache-query|find_joe');
```
